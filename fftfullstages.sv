  ...
  // Input stage
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
          ...
          stage_real[0][mic_input_index] <= in_real;
          stage_imag[0][mic_input_index] <= in_imag;
        ...
        for (genvar s = 0; s < STAGES; s++) begin : fft_stage
            ...
            always_ff @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                    ...
                    stage_valid[s+1] <= 1'b1;

                    for (int i = 0; i < N; i += BS) begin
                        for (int j = 0; j < HBS; j++) begin
                            idx_a[s] = i + j;
                            idx_b[s] = idx_a[s] + HBS;

                            twiddle_index[s] = (j * N) / BS;
                            ...
                            /*
                            prod_real[s] =
                            (b_real[s] * tw_r - b_imag[s] * tw_i) >>> (TWIDDLE_WIDTH - 1);
                            prod_imag[s] =
                            (b_real[s] * tw_i + b_imag[s] * tw_r) >>> (TWIDDLE_WIDTH - 1); */
                            prod_real[s] =
                            (b_real[s] * twiddle_real[twiddle_index[s]] - b_imag[s] * twiddle_imag[twiddle_index[s]]) >>> (TWIDDLE_WIDTH - 1);
                            prod_imag[s] =
                            (b_real[s] * twiddle_imag[twiddle_index[s]] + b_imag[s] * twiddle_real[twiddle_index[s]]) >>> (TWIDDLE_WIDTH - 1);

...
//               $display("s %d prod_imag %d", s, prod_imag[s]);
                ...
                // Butterfly
                stage_real[s+1][idx_a[s]] <= (a_real[s] + b_real[s])/2;
                stage_imag[s+1][idx_a[s]] <= (a_imag[s] + b_imag[s])/2;
                stage_real[s+1][idx_b[s]] <= (prod_real[s])/2;
                stage_imag[s+1][idx_b[s]] <= (prod_imag[s])/2;
...
//               stage_imag[s+1][group * (2**(s+1)) + pair + (2**s)] <= a_imag[s][group] - prod_imag[s];
    ...
    logic signed [DATA_WIDTH*3:0] max_magnitude;
    logic signed [DATA_WIDTH*3:0] current_magnitude;
